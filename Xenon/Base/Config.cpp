...
void _highlyExperimental::from_toml(const toml::value &value) {
  ...
  jitBackend = toml::find_or<std::string>(value, "JITBackend", jitBackend);
}
void _highlyExperimental::to_toml(toml::value &value) {
  ...
  value["JITBackend"].comments().clear();
  value["JITBackend"] = jitBackend;
  value["JITBackend"].comments().push_back("# JIT Backend selector (Legacy or HIR)");
}
bool _highlyExperimental::verify_toml(toml::value &value) {
  to_toml(value);
  cache_value(consoleRevison);
  cache_value(cpuExecutor);
  cache_value(jitBackend);
  from_toml(value);
  verify_value(consoleRevison);
  verify_value(cpuExecutor);
  verify_value(jitBackend);
  return true;
}
