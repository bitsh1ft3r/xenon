...
void CommandProcessor::WriteRegister(uint32_t index, uint32_t value) {
  ...
  if (index == 0x1844) { memcpy(fbAddressPtr ,&value, 4); }
  if (index == 0x184D) { memcpy(internalWidthPtr ,&value, 4); }
  if (index == 0x184E) { memcpy(internalHeightPtr,&value, 4); }
...
bool CommandProcessor::ExecutePacketType3_INTERRUPT(RingBuffer* reader,
  ...
  graphics_system_->DispatchInterruptCallback(1, cpu_mask);
  return true;
}
