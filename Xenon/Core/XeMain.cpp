...
#include "Core/HLE/System/HLESystemProvider.h"
#include "Core/HLE/Input/HLEInputProvider.h"
#include "Core/HLE/Input/XInputBackend/XInputBackend.h"
#include "Core/HLE/Video/HLEVideoProvider.h"
...
void XeMain::Create() {
  ...
  //
  // Register HLE Subsystems
  //

  auto &hleManager = Xe::Core::HLE::HLEFunctionManager::Get();

  // System: HLE system functions
  auto systemProvider = std::make_unique<Xe::Core::HLE::HLESystemProvider>();
  if (systemProvider->Setup()) {
    hleManager.AddProvider(std::move(systemProvider));
  }

  // Input: Windows Native XInput
  auto xinputBackend = std::make_unique<Xe::Core::HLE::XInputBackend>(ram.get());
  auto inputProvider = std::make_unique<Xe::Core::HLE::HLEInputProvider>(std::move(xinputBackend));
  if (inputProvider->Setup()) {
    hleManager.AddProvider(std::move(inputProvider));
  }

  // Video: HLE video functions
  auto videoProvider = std::make_unique<Xe::Core::HLE::HLEVideoProvider>();
  if (videoProvider->Setup()) {
    hleManager.AddProvider(std::move(videoProvider));
  }

  // Process all registered HLE functions into the CPU's table
  hleManager.RegisterAllFunctions(xenonCPU->GetCPUContext()->hleFunctionTable);
...
void XeMain::Shutdown() {
  ...
  // Clear HLE providers
  Xe::Core::HLE::HLEFunctionManager::Get().Clear();
...
void XeMain::CreatePCIDevices(RAM *ram) {
  ...
  audioController = std::make_shared<STRIP_UNIQUE(audioController)>("AUDIOCTRLR", AUDIO_CTRLR_DEV_SIZE, pciBridge.get(), ram);
  ...
  xma = std::make_shared<STRIP_UNIQUE(xma)>("XMA", XMA_DEV_SIZE, pciBridge.get(), ram);
