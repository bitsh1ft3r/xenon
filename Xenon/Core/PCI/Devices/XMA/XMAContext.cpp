...
#include "Core/PCI/Devices/XMA/XMATypes.h"
#include "Core/PCI/Devices/XMA/XMAContext.h"
...
// https://github.com/koolkdev/libertyv/blob/master/libav_wrapper/xma2dec.c
    ...
    int XmaContext::Setup(uint32_t id, RAM *ram, uint32_t guest_ptr) {
      id_ = id;
      ramPtr = ram;
      ...
      if (!av_codec_) {
        LOG_ERROR(XMA, "XmaContext {}: Codec not found", id);
        return 1;
      }

      av_context_ = avcodec_alloc_context3(av_codec_);
      if (!av_context_) {
        LOG_ERROR(XMA, "XmaContext {}: Couldn't allocate context", id);
        return 1;
      }
      ...
      if (!av_frame_) {
        LOG_ERROR(XMA, "XmaContext {}: Couldn't allocate frame", id);
        return 1;
      }
    ...
    bool XmaContext::Work() {
      ...
      auto context_ptr = ramPtr->GetPointerToAddress(guest_ptr());
    ...
    */
    ...
    bool XmaContext::ValidFrameOffset(uint8_t *block, size_t size_bytes,
      ...
      uint32_t first_frame_offset = GetPacketFrameOffset(packet);
    ...
    void XmaContext::Decode(XMA_CONTEXT_DATA *data) {
      ...
      uint8_t *in0 = data->input_buffer_0_valid
        ? ramPtr->GetPointerToAddress(data->input_buffer_0_ptr)
        : nullptr;
      uint8_t *in1 = data->input_buffer_1_valid
        ? ramPtr->GetPointerToAddress(data->input_buffer_1_ptr)
        : nullptr;
      ...
      // to the read offset.
      uint8_t *output_buffer = ramPtr->GetPointerToAddress(data->output_buffer_ptr);
      ...
      Xe::XGPU::RingBuffer output_rb(output_buffer, output_capacity);
      output_rb.setReadOffset(output_read_offset);
      output_rb.setWriteOffset(output_write_offset);
      ...
      size_t output_remaining_bytes = output_rb.writeCount();
      output_remaining_bytes -=
        output_remaining_bytes % (kBytesPerFrameChannel << data->is_stereo);

      // is_dirty_ = true; // TODO
      // is_dirty_ = false;  // TODO
      ASSERT(!data->stop_when_done);
      ASSERT(!data->interrupt_when_done);
      ...
      while (output_remaining_bytes > 0) {
        ...
        // ASSERT(packets_skip_ == 0);
        // ASSERT(split_frame_len_ == 0);
        // ASSERT(split_frame_len_partial_ == 0);
        ...
        if (split_frame_len_) {
          ...
          if (split_frame_len_ > kMaxFrameLength) {
          ...
          if (frame_count > 0) {
            ASSERT(GetPacketFrameOffset(packet) - 32 ==
          ...
          if (data->input_buffer_read_offset % kBitsPerPacket == 0) {
            ...
            auto offset =
              GetPacketFrameOffset(current_input_buffer +
            ...
            if (offset == -1) {
              ...
              LOG_ERROR(XMA, "XmaContext {}: TODO partial frames? end?", id());
              ASSERT("TODO");
              return;
            }
            else {
          ...
          if (split_frame_len_partial_ >= 15) {
            ...
            //ASSERT(true);
            split_frame_len_ = kMaxFrameLength + 1;
          }
          ASSERT(frame_is_split ==
            (split_frame_len_ > split_frame_len_partial_));
          ...
          if (frame_is_split) {
            ...
            packets_skip_ = GetPacketSkipCount(packet) + 1;
            while (packets_skip_ > 0) {
              packets_skip_--;
        ...
        if (ret < 0) {
          LOG_ERROR(XMA, "XmaContext {}: Error sending packet for decoding", id());
          // TODO bail out
         ASSERT(true);
        }
        ...
        */
        if (ret < 0) {
          LOG_ERROR(XMA, "XmaContext {}: Error during decoding", id());
         ASSERT(true);
...

        {
          ...
          //       current_frame_.size());
          ASSERT(av_context_->sample_fmt == AV_SAMPLE_FMT_FLTP);
          // ASSERT(frame_is_split == (frame_idx == -1));
          ...
          if (frame_idx + 1 >= frame_count) {
            ...
            packet = current_input_buffer + packet_idx * kBytesPerPacket;
            offset =
              GetPacketFrameOffset(packet) + packet_idx * kBitsPerPacket;
          }
          ...
          ASSERT(data->input_buffer_read_offset < offset);
          data->input_buffer_read_offset = offset;
        }
      }

      // ASSERT((split_frame_len_ != 0) == (data->input_buffer_read_offset ==
      // 0));
      ...
      if (output_rb.writeOffset() == output_rb.readOffset()) {
        data->output_buffer_valid = 0;
      }
    }
    ...
    std::tuple<int, int> XmaContext::GetFrameNumber(uint8_t *block, size_t size,
      ...
      auto first_frame_offset = GetPacketFrameOffset(packet);
      BitStream stream(block, size * 8);
    ...
    int XmaContext::PrepareDecoder(uint8_t *packet, int sample_rate,
      ...
      ASSERT((packet[2] & 0x7) == 1 || (packet[2] & 0x7) == 0);
      ...
      if (av_context_->sample_rate != sample_rate ||
        ...
        if (avcodec_open2(av_context_, av_codec_, NULL) < 0) {
          LOG_ERROR(XMA, "XmaContext: Failed to reopen FFmpeg context");
          return -1;
        }
        return 1;
      }
    ...
    uint64_t BitStream::Peek(size_t num_bits) {
      ...
      ASSERT(num_bits <= 57);
      ASSERT(offset_bits_ + num_bits <= size_bits_);
