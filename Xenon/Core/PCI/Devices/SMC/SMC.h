...
// DDC I2C Controller State
// Emulates the HANA DDC master accessed via SMC FIFO subcommands 0x11/0x21/0x81.
// Only registers actually accessed by VdpHanaDdc* kernel functions are modelled:
//   0x1EC  busConfig  — bus recovery clock-line control (VdpHanaDdcBusRecover)
//   0x1F2  busStatus  — bit 2=FIFO_DONE, bit 4=BUSY, bits 5-6=errors
//   0x1F3  ctrlReg    — 0x09=prime/enable, 0x0F=full reset, 0x0A=recover step 2
//   0x1ED  (7-byte write via cmd 0x81) — DDC master command, triggers I2C read
//   0x1F4  (multi-byte read via cmd 0x11) — FIFO data drain (≤13 bytes/call)
//   0x1F5  (single-byte read via cmd 0x11) — bytes currently in FIFO
struct DDC_STATE {
  u8 busConfig = 0;    // 0x1EC
  u8 busStatus = 0;    // 0x1F2: bit 2=FIFO_DONE, bit 4=BUSY, bits 5-6=error
  u8 ctrlReg = 0;      // 0x1F3

  // FIFO buffer populated when a DDC master command (0x1ED write) executes.
  // Max one full EDID block (128 bytes).
  u8  fifoBuf[128] = {};
  u8  fifoReadPos = 0; // next byte index to dequeue from fifoBuf
  u8  fifoCount = 0;   // bytes remaining in fifoBuf
};
...
struct SMC_CORE_STATE {
  ...
  // DDC I2C controller state for EDID reads
  DDC_STATE ddcState = {};
