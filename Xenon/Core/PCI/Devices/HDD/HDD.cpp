...
void Xe::PCIDev::HDD::ataWriteDMACommand() {
  ...
  // Defer the actual write to doDMA(), where the data arrives via PRD transfers.
  ataState.writeOffset = offset;
  ataState.sectorCount = sectorCount;
}
...
void Xe::PCIDev::HDD::doDMA() {
  for (;;) {
    ...
    if (readOperation) {
      ...
      // Writing to us
      ataState.dataInBuffer.init(size, true);
      ataState.dataInBuffer.reset();
      memcpy(ataState.dataInBuffer.get(), bufferInMemory, size);

      ataState.mountedHDDImage->Write(ataState.writeOffset, ataState.dataInBuffer.get(), size);
      ataState.writeOffset += size;
    }
    if (lastEntry) {
