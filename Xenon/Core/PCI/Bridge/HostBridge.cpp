...
bool HostBridge::Read(u64 readAddress, u8 *data, u64 size) {
  ...
  // Determine target device without holding the lock for the actual call.
  // This avoids nested lock contention when child devices have their own mutexes.
  enum class Target { None, HostBridge, XGPU, PCIBridge };
...

  {
    ...
    // Determine which child device should handle this address
    if (xGPU->IsAddressMappedInBAR(static_cast<u32>(readAddress))) {
      target = Target::XGPU;
    } else if (pciBridge->IsAddressMappedinBAR(static_cast<u32>(readAddress))) {
      target = Target::PCIBridge;
    }
  } // Release lock before calling devices

  // Call child device methods without holding the HostBridge mutex
  switch (target) {
  case Target::XGPU:
    xGPU->Read(readAddress, data, size);
    return true;
  case Target::PCIBridge:
...
bool HostBridge::Write(u64 writeAddress, const u8 *data, u64 size) {
...

  {
    ...
    if (xGPU->IsAddressMappedInBAR(static_cast<u32>(writeAddress))) {
      target = Target::XGPU;
    } else if (pciBridge->IsAddressMappedinBAR(static_cast<u32>(writeAddress))) {
      target = Target::PCIBridge;
    }
  } // Release lock before calling child devices
...
bool HostBridge::ConfigRead(u64 readAddress, u8 *data, u64 size) {
  ...
  // Determine target device without holding the lock for the actual call.
  enum class Target { None, HostBridge, XGPU, PCIBridge, SecondaryBus };
  Target target = Target::None;
