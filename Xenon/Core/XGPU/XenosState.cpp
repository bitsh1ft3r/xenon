...
#endif
{
  ...
  fbAddressPtr = reinterpret_cast<u8*>(&fbSurfaceAddress);
  internalWidthPtr = reinterpret_cast<u8*>(&internalWidth);
  internalHeightPtr = reinterpret_cast<u8*>(&internalHeight);
}
...
#ifdef XENIA_D3D12_GPU
  // Forward every register write to xenia's D3D12 backend.
  if (xeniaD3D12Bridge && xeniaD3D12Bridge->IsActive()) {
    tmp = xeniaD3D12Bridge->ForwardRegisterRead(regIndex);
  }
#endif
...
void Xe::XGPU::XenosState::WriteRawRegister(u32 addr, u32 value) {
  ...
  // Forward every register write to xenia's D3D12 backend.
  if (xeniaD3D12Bridge && xeniaD3D12Bridge->IsActive()) {
    xeniaD3D12Bridge->ForwardRegisterWrite(regIndex, value);
  }
#endif
