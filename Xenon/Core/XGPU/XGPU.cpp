...
#ifdef XENIA_D3D12_GPU
  // Create and initialize the Xenia D3D12 bridge.
  xeniaD3D12Bridge = std::make_unique<Xe::Xenos::XeniaD3D12Bridge>(ramPtr, parentBus);
  if (xeniaD3D12Bridge->Initialize()) {
    xenosState->xeniaD3D12Bridge = xeniaD3D12Bridge.get();
  } else {
    LOG_ERROR(Xenos, "Failed to initialize XeniaD3D12Bridge");
    xeniaD3D12Bridge.reset();
  }
#endif
...
#ifdef XENIA_D3D12_GPU
  // Shut down the Xenia D3D12 bridge before the command processor.
  if (xeniaD3D12Bridge) {
    xenosState->xeniaD3D12Bridge = nullptr;
    xeniaD3D12Bridge.reset();
  }
#endif
...
void Xe::Xenos::XGPU::xeVSyncWorkerThreadLoop() {
  ...
  constexpr duration<double> VSyncInterval = duration<double>(1.0 / 60.0);

  auto nextVblank = clock::now() + VSyncInterval;
  ...
  while (xeVsyncWorkerThreadRunning) {
    ...
    if (xenosState->d1modeIntMask & 0x40000011) {
      // Set  VBLANK Pending
      xenosState->vblankVlineStatus |= 0x11000100; // Hardware dump shows this (byteswapped) value at interrupt time.
      xenosState->d1modeIntMask &= ~0x40000011;
      parentBus->RouteInterrupt(PRIO_GRAPHICS, 4); // Debugging on hardware shows #2 is the correct CPU ID.
    }

    // Update timer and catch up
    nextVblank += VSyncInterval;

    if (nextVblank < clock::now()) {
      nextVblank = clock::now() + VSyncInterval;
    }
  }
