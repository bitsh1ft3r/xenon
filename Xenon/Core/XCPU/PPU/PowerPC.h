...
struct sPPUThread {
  ...
  // Set to true when an interrupt is handled (system enters ISR routine in HV). Used to not re issue sync exceptions when one is already being processed.
  bool interruptTaken = false;

  // Block-linking chain budget. Decremented by HIR-emitted chain tails before
  // each indirect call into the next block. When it reaches zero the tail
  // returns to the dispatcher, breaking unbounded recursion through cycles
  // (B->C->B and similar). Reset to a fixed value at the top of each
  // ExecuteJITInstrs iteration.
  u32 jitChainBudget = 0;

  // Set by the IIC worker thread when a deliverable external interrupt arrives.
  // Only the IIC worker thread sets this to true; only the owning PPU thread clears it.
  alignas(64) std::atomic<bool> externalInterruptPending{ false };
};
...
enum class ePPUTestingMode : u8 {
  ...
  JITx86,      // X86 JIT mode
  HIR,         // PPC -> HIR -> Optimization -> Backend code (x86/ARM) translation 
};

// TimeBase structure for tracking timebase increments and decrementer scheduling.
struct sTimeBaseState {
