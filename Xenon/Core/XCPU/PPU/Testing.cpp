...
public:
  ...
  bool Run(TestCase &testCase) {
    ...
    if (currentTestMode == ePPUTestingMode::Interpreter) {
      ...
      std::atomic<eThreadState> threadState = eThreadState::Running;
      ppuJIT->ExecuteJITInstrs(0x100, threadState, ppeState->currentThread, false, true);
    } else if (currentTestMode == ePPUTestingMode::HIR) {
      sPPUThread &thread = ppeState->ppuThread[ppeState->currentThread];
      void *code = nullptr;
      u64 codeSize = 0;
      if (ppuJIT->BuildHIRBlock(thread.NIA, 64, ppeState->currentThread, &code, &codeSize)) {
        // Execute the compiled HIR block. The function signature is now
        // void(PPU*, sPPEState*, bool), matching JITFunc.
        // PPU* (arg0) is unused, sPPEState* (arg1) is used to derive thread context.
        auto fn = reinterpret_cast<JITFunc>(code);
        fn(nullptr, ppeState, false);

        // Release the code after execution.
        ppuJIT->ReleaseHIRCode(code);
      } else {
        LOG_ERROR(Xenon, "[Testing]: HIR block compilation failed at {:#x}", thread.NIA);
      }
    }
  ...
  bool SetupTestState(TestCase &testCase) {
    ...
    for (auto &it : testCase.testAnnotations) {
      if (it.first == "REGISTER_IN") {
        ...
        auto p = PPCInterpreter::MMUGetPointerFromRAM(address, ppeState);
  ...
  bool CheckTestResults(TestCase &testCase) {
    ...
    for (auto &it : testCase.testAnnotations) {
      if (it.first == "REGISTER_OUT") {
        ...
        auto bytesStr = it.second.substr(spacePos + 1);
        u32 address = std::strtoul(addressStr.c_str(), nullptr, 16);
        auto baseAddress = PPCInterpreter::MMUGetPointerFromRAM(address, ppeState);
        auto p = baseAddress;
        const char *c = bytesStr.c_str();
        bool failed = false;
        size_t count = 0;
