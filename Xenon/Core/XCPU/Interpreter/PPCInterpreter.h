...
// Current thread ID - thread local variable
...
// Notify RAM observers (GPU shared-memory tracking, JIT block invalidation, ...)
// of a write that was performed inline by the JIT through a fastDataCache hit.
// |hostPtr| is the address actually written to; |byteCount| is the access width.
// Also clears any reservation that covers the physical address. Safe to call
// for any host pointer; if it is outside the RAM mapping, the call is a no-op.
void JITNotifyWrite(u8 *hostPtr, u32 byteCount);
...
u8 *MMUGetPointerFromRAM(u64 EA, sPPEState *ppeState, bool translate = false);
...
// Fast-path: resolve EA to host pointer for RAM-backed accesses.
// Returns host pointer on success, nullptr for SoC/MMIO or translation failure.
// Caller must fall back to MMURead/MMUWrite on nullptr.
u8 *mmuResolveHostPtr(sPPEState *ppeState, u64 EA, bool write, ePPUThreadID thr = ePPUThread_None);
