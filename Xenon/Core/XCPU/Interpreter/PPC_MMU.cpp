  ...
  if (invalSelector) {
    ...
    if (XeMain::GetCPU()) {
      ...
      if (ppu && ppu->GetPPUJIT()) {
        DEBUGP("[TLBIEL]: Congruence-class invalidation (class {:#x})", classIndex);
        ppu->GetPPUJIT()->InvalidateAllBlocks(curThreadId);
      }
    }
  } else {
    ...
    for (u8 way = 0; way < 4; ++way) {
      ...
      if (entry.V && ((entry.VPN & compareMask) == (rb & compareMask))) {
        DEBUGP("[TLB]: TLBIEL: Invalidating entry at class {} way {} VPN: {:#x}",
    ...
    if (XeMain::GetCPU()) {
      ...
      if (ppu && ppu->GetPPUJIT()) {
        ...
        DEBUGP("[TLBIEL]: Invalidating JIT blocks for page {:#x} (size {:#x})", start, pageSize);
        ppu->GetPPUJIT()->InvalidateBlocksForRange(start, end, curThreadId);
      }
    }
  }
}
...
void PPCInterpreter::PPCInterpreter_tlbie(sPPEState *ppeState) {
  ...
  if (Config::log.advanced)
    DEBUGP("tlbie, EA:0x{:X} | PageSize:{} | Full:0x{:X} | LP:{}",
  ...
  if (XeMain::GetCPU()) {
    ...
    if (ppu && ppu->GetPPUJIT()) {
      ...
      u64 end = start + pageSize;
      DEBUGP("[TLBIE]: Invalidating JIT blocks for page {:#x} (size {:#x})", start, pageSize);
      ppu->GetPPUJIT()->InvalidateBlocksForRange(start, end, curThreadId);
    }
  }
}
...
void PPCInterpreter::mmuAddTlbEntry(sPPEState *ppeState) {
  ...
  DEBUGP("[TLB]: Adding entry: Class: {:#x}, Set: {:#b}, VPN: {:#x}, RPN: {:#x}",
    TI, TS, VPN, RPN);
...
void PPCInterpreter::mmuAddTlbEntryHardware(sPPEState *ppeState, u64 VA,
  ...
  DEBUGP("[TLB]: Hardware reload: Class: {:#x}, Way: {}, VPN: {:#x}, RPN: "
         "{:#x}, p: {}",
...
u64 PPCInterpreter::JITTranslateAndGetHostPtr(sPPEState *ppeState, u64 EA, ePPUThreadID thr, bool write) {
  ...
  if (!MMUTranslateAddress(&returnedAddr, ppeState, false, thr)) {
    DEBUGP("[JIT MMU]: Address translation failed for EA: {:#x}", EA);
    return 0;
  } 
  ...
  if (!hostPtr) {
    ...
    if (!socRead && returnedAddr <= PHYS_MEMORY_END && hostPtr) {
      ...
      if (write) {
        // Notify RAM
        RAM *ram = PPCInterpreter::xenonContext->GetRAM();
        ram->NotifyWrite(returnedAddr, static_cast<u32>(1));
      }    
    }
  }
...
// Notify RAM of a write performed inline by the JIT through a fastDataCache hit.
// Mirrors the bookkeeping the slow-path MMUWrite does: reservation check +
// observer callback. Only the RAM range produces a notification; pointers
// outside RAM (shouldn't happen for cache hits, but defensive) are ignored.
void PPCInterpreter::JITNotifyWrite(u8 *hostPtr, u32 byteCount) {
  if (!hostPtr || !PPCInterpreter::xenonContext) {
    return;
  }
  RAM *ram = PPCInterpreter::xenonContext->GetRAM();
  if (!ram) {
  ...
  u8 *ramBase = ram->GetRamBase();
  const u64 size = ram->GetSize();
  const ptrdiff_t off = hostPtr - ramBase;
  if (off < 0 || static_cast<u64>(off) >= size) {
    return;
  }
  const u32 physAddr = static_cast<u32>(off);
  PPCInterpreter::xenonContext->xenonRes.Check(physAddr);
  ram->NotifyWrite(physAddr, byteCount);
}
...
#ifdef DEBUGP
        if (Config::log.advanced)
          DEBUGP("Checking valid SLB (V:0x{:X},LP:0x{:X},C:0x{:X},L:0x{:X},N:0x{:X},Kp:0x{:X},Ks:0x{:X},VSID:0x{:X},ESID:0x{:X},vsidReg:0x{:X},esidReg:0x{:X})", static_cast<u32>(slbEntry.V), static_cast<u32>(slbEntry.LP), static_cast<u32>(slbEntry.C), static_cast<u32>(slbEntry.L),
...
u8* PPCInterpreter::MMUGetPointerFromRAM(u64 EA, sPPEState *ppeState, bool translate) {
  if (translate) {
    u64 RA = EA;
    if (MMUTranslateAddress(&RA, ppeState, true)) {
      bool socWrite = false;
      RA = mmuContructEndAddressFromSecEngAddr(RA, &socWrite);
      return xenonContext->GetRAM()->GetPointerToAddress(RA);
    } else {
      LOG_ERROR(Xenon, "Failed to translate address {:#x} in MMUGetPointerFromRAM", EA);
      // Clean exception from failed translation
      auto &thread = curThread;
      thread.exceptReg &= ~ppuDataStorageEx;
      return nullptr;
    }
  } else { return xenonContext->GetRAM()->GetPointerToAddress(EA); }
