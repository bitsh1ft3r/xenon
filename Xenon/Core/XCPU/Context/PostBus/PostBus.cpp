...
void Xe::XCPU::POSTBUS::POST(u64 postCode) {
  ...
  /* KERNEL */ // Kernel post codes vary according to each system version. Lets rely
    ...
    {
      u8 *imgBasePtr = XeMain::ram->GetPointerToAddress(HLE::XBOXKRNL_IMAGE_BASE);
      XeMain::GetCPU()->GetCPUContext()->xboxkrnlImageParser.ParseImageData(imgBasePtr, HLE::XBOXKRNL_HEADER_SIZE);
      // Parse HLE Function table
      XeMain::GetCPU()->GetCPUContext()->ProcessHLEFunctionTable(Xe::Core::HLE::eSystemExecutables::xboxkrnl);
    }
  }
