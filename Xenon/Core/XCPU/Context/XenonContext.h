...
namespace Xe::XCPU {
  ...
  public:
    ...
    HLE::PEImageParser xboxkrnlImageParser;
    // PE Image parser for resolving xam ordinals onto relative RAM addresses.
    HLE::PEImageParser xamImageParser;

    // Called after a given system executable loads, uses PEImageParser to parse and assign all entries in the hleFunctionTable 
    // and assigns them a Virtual address.
    void ProcessHLEFunctionTable(Core::HLE::eSystemExecutables executable);

    // High level emulated function table. Contains all HLE functions 
    std::vector<Xe::Core::HLE::sHLEFunction> hleFunctionTable;
