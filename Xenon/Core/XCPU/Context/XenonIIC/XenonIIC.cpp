...
void Xe::XCPU::XenonIIC::Write(u64 writeAddress, const u8* data, u64 size) {
  ...
  if (offset < ProcessorBlocksEnd) {
    ...
    case 0x0060: // EndOfInterrupt
      // Erase the highest-priority ACK'd interrupt (lock-free)
      {
...
// Fully lock-free O(1) operation using atomic bitmasks.
...
// Removes the highest-priority ACK'd interrupt (EOI) for a given thread. Lock-free O(1).
...
u8 Xe::XCPU::XenonIIC::acknowledgeInterrupt(u8 threadID) {
  ...
  while (true) {
    ...
    // std::countl_zero gives the number of leading zeros; we use it to find the MSB.
