...
// Processes all entries in hleFunctionTable assigning a VA to all functions that match a given ordinal within a system executable
void XenonContext::ProcessHLEFunctionTable(Core::HLE::eSystemExecutables executable) {
  // Check if table is empty
  if (hleFunctionTable.empty()) { return; }

  // Populate all guest addresses for valid ordinals belonging to valid system executbales
  for (auto &function : hleFunctionTable) {
    // Only search for functions with a valid ordinal and function pointer
    if (function.ordinal != INVALID_ORDINAL && function.functionPtr != nullptr) {
      if ((function.guestExecutable == Core::HLE::eSystemExecutables::xboxkrnl) && executable == Core::HLE::eSystemExecutables::xboxkrnl) {
        function.guestAddress = xboxkrnlImageParser.getAbsVAByOrdinal(function.ordinal);
      } else if ((function.guestExecutable == Core::HLE::eSystemExecutables::xam) && executable == Core::HLE::eSystemExecutables::xam) {
        function.guestAddress = xamImageParser.getAbsVAByOrdinal(function.ordinal);
      } else {
        LOG_ERROR(HLE, "Invalid system executable for HLE function {}. Ordinal {:#d}.", function.functionName, function.ordinal);
        //function.guestAddress = 0;
      }
      
      if (function.guestAddress != 0) {
        // Set function valid
        function.valid = true;

        LOG_DEBUG(HLE, "Validated HLE function {} from function table. Ordinal {:#d}, VA {:#x}", function.functionName, function.ordinal, function.guestAddress);
      } else{
        LOG_DEBUG(HLE, "Failed to validate HLE function {} from function table. Ordinal {:#d}.", function.functionName, function.ordinal);
      }
    }
...
bool XenonContext::HandlePMWRead(u64 readAddr, u8 *data, size_t byteCount) {
  ...
  dataOut = byteswap_be<u64>(dataOut);
  memcpy(data, &dataOut, byteCount);
  // Disable debug output due to PIX massively reading regs thus causing slowdown.
...
bool XenonContext::HandlePMWWrite(u64 writeAddr, const u8 *data, size_t byteCount) {
  ...
  // Disable debug output due to PIX massively writing regs thus causing slowdown.
