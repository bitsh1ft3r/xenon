...
void PPCInterpreter::PPCInterpreterJIT_rfid(sPPEState* ppeState, JITBlockBuilder* b, uPPCInstr instr) {
  ...
  x86::Gp exceptFlag = newGP8();
  COMP->mov(exceptFlag, b->threadCtx->scalar(&sPPUThread::interruptTaken));
  COMP->xor_(exceptFlag, exceptFlag);
  COMP->mov(b->threadCtx->scalar(&sPPUThread::interruptTaken), exceptFlag);
}
